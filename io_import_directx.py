...
from bel.io_bdata import createMeshObject
...
if format == 'txt' :
    ...
    '''            
                ...
                tree = tree[0:lvl]
                parent = tree[-1]
                token[frame] = {'pointer': ptr, 'line' : c, 'parent': parent, 'childs':[], 'type':'bone'}
                tree.append(frame)
                if lvl > 1 : token[parent]['childs'].append(frame)
                
            elif re.match(r_matrix,l) :
                token[frame]['matrix'] = c
                ...
                #if 'meshes' not in token[ frame] : token[frame]['meshes'] = {mesh: mdta }
                #else : token[frame]['meshes'][mesh] = mdta
                token[mesh] = mdta
                tree.append(mesh)
                #print(mesh,token[mesh])
                if lvl > 1 :
                    token[parent]['childs'].append(mesh)
                    #print(parent,token[parent])
    ...
    #for l in data.readlines() :
        ...
        for l in lines :
            ...
            if re.match(r_template,l) :
                ...
                token[parent]['childs'].append('*'+refname) 
                if refname not in token :
                    print('reference to %s done before its declaration (line %s)\ncreated dummy'%(refname,c))
                    token[refname] = {}
                if 'user' not in token[refname] : token[refname]['users'] = [parent]
                else : token[refname]['users'].append(parent)
                ...
                parent = tree[-1]
                if mesh in token :
                    token[mesh]['pointer'] = ptr
                    token[mesh]['line'] = c
                    token[mesh]['parent'] = parent
                    token[mesh]['childs'] = []
                    token[mesh]['type'] = typ
                    
                else : token[mesh] = {'pointer': ptr, 'line' : c, 'parent':parent, 'childs':[], 'users':[], 'type':typ}
                tree.append(mesh)
                if lvl > 1 :
                    token[parent]['childs'].append(mesh)
    ...
    def walk_dxtree(field,lvl,tab='') :
        for fi, framename in enumerate(field) :
            if lvl > 0 or token[framename]['parent'] == '' :
                if framename not in token :
                    framename = framename[1:]
                    ref = 'ref: '
                else : ref = False
                
                frame_type = token[framename]['type']
                line = ('{:7}'.format(token[framename]['line']))
                log = ' %s%s (%s)'%( ref if ref else '', framename, frame_type )
                print('%s.%s%s'%(line, tab, log))
                if fi == len(field) - 1 : tab = tab[:-3] + '   '

                if ref == False :
                    for user in token[framename]['users'] :
                         print('%s.%s |__ user: %s'%(line, tab.replace('_',' '), user))
                    walk_dxtree(token[framename]['childs'],lvl+1,tab.replace('_',' ')+' |__')
                
                if fi == len(field) - 1 and len(token[framename]['childs']) == 0 :
                    print('%s.%s'%(line,tab))
    
    
    print('\nTREE TEST\n')
    walk_dxtree(token.keys(),0)
